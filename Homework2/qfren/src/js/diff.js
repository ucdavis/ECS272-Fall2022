...
function null_filter(arr) {
    ...
    arr.forEach(d => {
      let f = 0;
      Object.keys(d).forEach((key) => (d[key] === null || d[key] === 0)? f = f + 1 : f);
      if (f === 0){
        res.push(d);
      }
    })
  ...
  export async function drawDiff(chartid){
    d3.csv(csvPath).then((d) => {
      ...
      DivergingBarChart(data, {
        x: d => d[2019] - d[1990],
        y: d => d["Country Name"],
        yDomain: d3.groupSort(data, ([d]) => d[2019] - d[1990], d => d['Country Name']),
        xFormat: "+d",
        xLabel: "← decrease · Change in Carbon Dioxide Emission · increase →",
