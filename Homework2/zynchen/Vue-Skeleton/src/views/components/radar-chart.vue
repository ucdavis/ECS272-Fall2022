<template>
  <svg class="radar-chart" :width="width" :height="height">
    <g :transform="`translate(${padding}, ${padding})`">
      <g :transform="`translate(0, ${descriptionHeight})`">
        <text class="dimension-label" v-for="(label, i) in dimensions" :transform="`translate(${(radarRaidus + textRasius * Math.sin(angles[i]) - padding/3).toFixed()}, ${(radarRaidus - textRasius * Math.cos(angles[i]) + padding/6).toFixed()})`" @click="onClickDimensionLabel(i)">{{label}}</text>
        ...
        <text v-for="(v, i) in netValues" :transform="`translate(${radarRaidus + 4}, ${radarRaidus * (15-v)/15})`">{{v}}</text>
      </g>
      <g class="group" v-for="item in dataPolygons" @click="onClickGroup(item.index)" v-on:mouseover="onGroupHover(item.index)">
        <g class="label" :transform="`translate(${90*item.index}, 0)`">
          <rect :x="0" :y="0" :width="20" :height="8" :fill="dataset[item.index].color"></rect>
          <text :x="30" :y="8" fill="#000">{{dataset[item.index].label}}</text>
        </g>
        <g :transform="`translate(0, ${descriptionHeight})`">
