<template>
    <div class="lineChartContainer">
        <div class="optionBar">
            ...
            <!-- <label id="countryLabel">Please click on the World Map to select a country.</label> -->
            <button class="resetBtn" type="button" @click="drawLine">Reset Brush</button>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                yearfield: ["1990","1991","1992","1993","1994",
        ...
        mounted(){
            ...
            console.log("Data Passed down as a Prop  ", this.selected_country);
        ...
        methods: {
            ...
            init(id){
                ...
                svg.append("text").attr("id", "YLabel");
                svg.append("defs").append("svg:clipPath")
                    ...
                    .attr("x", 0)
                    .attr("y", 0);
                this.scatter = svg.append("g").attr("clip-path", "url(#clip)");
                // this.scatter.append("path").attr("id", "lineGroup");
            ...
            drawLine() {
                // if (this.selected_country != ""){this.country = this.selected_country;};
                this.localData = this.convertDataFormat(this.myLineChartData, this.country);
                let data = this.localData;
                let vueThis = this;
                // let svg = d3.select("#lineGroup");
                // document.getElementById("countryLabel").innerHTML = this.country;

                let x_axis = d3.scalePoint()
                    .domain(this.yearfield)
                ...
                let xAxis = d3.select("#lineXAxisGroup")
                    .attr("class", "x-axis")
                    .attr("transform", `translate(0,${this.height - this.margin.bottom})`)
                
                xAxis.call(d3.axisBottom(x_axis))
                    .selectAll("text")
                ...
                let brush = d3.brushX().extent( [ [0,0], [this.width, this.height] ] ).on("end", updateChart);
                ...
                function updateChart() {
                    function idled() { vueThis.idleTimeout = null; };
                    function scalePointPosition(pos) {
                        let range = x_axis.range();
                        let rangePoints = d3.range(range[0], range[1], x_axis.step());
                        let yPos = d3.bisect(rangePoints, pos);
                        return yPos
                    };
                    let extent = d3.event.selection;

                    if(!extent){
                        if (!vueThis.idleTimeout) return vueThis.idleTimeout = setTimeout(idled, 350); 
                        x_axis.domain(vueThis.yearfield);
                    }else{
                        x_axis.domain(vueThis.yearfield.slice(scalePointPosition(extent[0]), scalePointPosition(extent[1])));
                        vueThis.scatter.select(".brush").call(brush.move, null);
                    };
