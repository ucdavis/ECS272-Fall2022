<template>
    <a-row type="flex">
        ...
        <a-col :flex="1">
            <div>
                <label>Order: </label>
                <a-select v-model:value="order" style="width: 140px" @change="handleChange">
                    <a-select-option :value="item" v-for="item in orderList">
                        {{item}}
                    </a-select-option>
                </a-select>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            chart: undefined,
            orderMap: (a, b) => d3.ascending(a.letter, b.letter),
        }
    },
    props:{
        myBarChartData: Object,
    },
    mounted(){
        console.log("Data Passed down as a Prop  ", this.myBarChartData);
        this.chart = this.drawBarChart(this.myBarChartData, '#bar', {
            x: d => 'Gen ' + d.generation,
            y: d => d.stats_avg,
            yLabel: "↑ Avg Stats",
            color: "steelblue",
    ...
    methods: {
        handleChange() {
            if (this.order === 'Generation') {
                this.orderMap = (a, b) => d3.ascending(a.generation, b.generation);
            } else if (this.order === 'Ascending') {
                this.orderMap = (a, b) => d3.ascending(a.stats_avg, b.stats_avg);
            } else {
                this.orderMap = (a, b) => d3.descending(a.stats_avg, b.stats_avg)
            }
            this.chart.update(d3.sort(this.myBarChartData, this.orderMap));
        },
