            ...
            drawBarChart(dset, id, option) {
                ...
                const bars = svg.append('g')
                    ...
                    .on('mouseover', (e, d) => {
                        ...
                        bars.attr('fill', '#ECEBEB')
                            .filter(dd => {return dd.region == d.region || focused.has(dd.region)})
                        ...
                        bars.attr('fill', '#ECEBEB')
                            .filter(dd => {
                                if (focused.size == 0){
                                    ...
                                    return focused.has(dd.region)
                                }
                            })
                            .attr('fill', dd => colors(dd.region))
                    })
                    .on('click', (e, d) => {
                        if (focused.has(d.region)){
                            focused.delete(d.region)
                        } else {
                            focused.add(d.region)
                        }
                ...
                svg.append('text')
                    .text('click bar to select/deselect')
                    .attr('x', 10)
                    .attr('y', 20)
                    .attr('font-size', 8)
